...
<body class="selection:bg-terracotta-500 selection:text-white">
    ...
    <header class="sticky top-0 z-40 bg-[#FBF8F3]/90 backdrop-blur-md editorial-rule">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 py-3 flex items-center justify-between">
            ...
            <nav class="hidden lg:flex items-center space-x-1 text-sm font-medium text-[#575249]">
                ...
                <a href="#cross-comparison" class="px-3 py-1.5 rounded hover:text-[#C04A26] hover:bg-[#F4EFE6] transition font-bold text-[#C04A26]">三大事件深度圖表</a>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-20">
            ...
            <div class="lg:col-span-7 space-y-6">
                ...
                <div class="pt-2 flex flex-wrap gap-4 text-xs font-mono">
                    <span class="px-3 py-1.5 rounded bg-[#F4EFE6] text-[#26211C] border border-[#E6DEC8]">
                        <i class="fa-solid fa-layer-group text-[#C04A26] mr-1"></i> 全套 20 組專業圖表
                    </span>
                    <span class="px-3 py-1.5 rounded bg-[#F4EFE6] text-[#26211C] border border-[#E6DEC8]">
        ...
        <section id="cross-comparison" class="space-y-8">
            ...
            <div class="flex flex-wrap gap-2 pt-2" id="gallery-tabs">
                <!-- 動態注入或預設 8 大代表性對照主題 -->
                <button onclick="setTheme(1)" id="tab-1" class="custom-tab-active px-3.5 py-2 rounded text-xs sm:text-sm font-medium border border-[#E6DEC8] transition bg-white text-[#26211C]">
                    1. 初至波形橫向對比
                </button>
                <button onclick="setTheme(2)" id="tab-2" class="px-3.5 py-2 rounded text-xs sm:text-sm font-medium border border-[#E6DEC8] transition bg-white text-[#26211C]">
                    2. 時頻能量譜三聯對照
                </button>
                <button onclick="setTheme(3)" id="tab-3" class="px-3.5 py-2 rounded text-xs sm:text-sm font-medium border border-[#E6DEC8] transition bg-white text-[#26211C]">
                    3. 地波/音爆能量分配柱狀圖
                </button>
                <button onclick="setTheme(4)" id="tab-4" class="px-3.5 py-2 rounded text-xs sm:text-sm font-medium border border-[#E6DEC8] transition bg-white text-[#26211C]">
                    4. 質點運動軌跡 (Hodogram)
                </button>
                <button onclick="setTheme(5)" id="tab-5" class="px-3.5 py-2 rounded text-xs sm:text-sm font-medium border border-[#E6DEC8] transition bg-white text-[#26211C]">
                    5. 走時曲線與音速迴歸
                </button>
                <button onclick="setTheme(6)" id="tab-6" class="px-3.5 py-2 rounded text-xs sm:text-sm font-medium border border-[#E6DEC8] transition bg-white text-[#26211C]">
                    6. 屏東明揚雙爆轟能量放大
                </button>
                <button onclick="setTheme(7)" id="tab-7" class="px-3.5 py-2 rounded text-xs sm:text-sm font-medium border border-[#E6DEC8] transition bg-white text-[#26211C]">
                    7. 麥寮超壓衝擊波 N 波解析
                </button>
                <button onclick="setTheme(8)" id="tab-8" class="px-3.5 py-2 rounded text-xs sm:text-sm font-medium border border-[#E6DEC8] transition bg-white text-[#26211C]">
                    8. 全臺氣爆地理分佈總圖
                </button>
            </div>
        ...
        <section id="kaohsiung-study" class="space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                <div class="space-y-4 text-xs sm:text-sm text-[#575249] leading-relaxed font-serifBody">
                    ...
                    <p>
                        ...
                        隨著震央距延伸至 20 km 以上（如 SNJ 24.8 km），地殼波先於第 7 秒抵達，大氣音爆則在第 73 秒才抵達，但音爆撞擊地表激發的空地耦合波振幅高達地波的 6.5 倍！
                    </p>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <p class="text-xs text-center text-[#686055] font-mono">
                        圖 1: GMT 繪製之高雄氣爆走時剖面 (0~45 km)，近場到遠場雙走時射線完整復刻 Yamada 2014 第 6 頁成果
                    </p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
                <div class="bg-white p-3 rounded-lg border border-[#E6DEC8] cursor-pointer hover:border-[#C04A26] transition" onclick="openModal('./GMT繪圖成果/GMT_02_高雄氣爆_原始與帶通濾波對比圖.png', '原始波形 vs 2-8 Hz 帶通濾波對比')">
                    <img src="./GMT繪圖成果/GMT_02_高雄氣爆_原始與帶通濾波對比圖.png" class="w-full h-36 object-cover rounded">
                    <div class="mt-2 text-xs font-bold text-[#26211C] font-display">原始 vs 帶通濾波</div>
                    <p class="text-[11px] text-[#686055] mt-0.5 font-serifBody">凸顯 2-8 Hz 頻率窗內初至訊號</p>
                </div>
                <div class="bg-white p-3 rounded-lg border border-[#E6DEC8] cursor-pointer hover:border-[#C04A26] transition" onclick="openModal('./GMT繪圖成果/GMT_03_高雄氣爆_近場三分量時序圖.png', 'KAU & SGL 近場三分量加速度波形')">
                    <img src="./GMT繪圖成果/GMT_03_高雄氣爆_近場三分量時序圖.png" class="w-full h-36 object-cover rounded">
                    <div class="mt-2 text-xs font-bold text-[#26211C] font-display">近場三分量時序</div>
                    <p class="text-[11px] text-[#686055] mt-0.5 font-serifBody">KAU 與 SGL 兩站六分量 PGA 標註</p>
                </div>
                <div class="bg-white p-3 rounded-lg border border-[#E6DEC8] cursor-pointer hover:border-[#C04A26] transition" onclick="openModal('./GMT繪圖成果/GMT_06_高雄氣爆_近場質點運動極化軌跡圖.png', '近場質點運動極化圖 (Hodogram)')">
                    <img src="./GMT繪圖成果/GMT_06_高雄氣爆_近場質點運動極化軌跡圖.png" class="w-full h-36 object-cover rounded">
                    <div class="mt-2 text-xs font-bold text-[#26211C] font-display">質點運動極化圖</div>
                    <p class="text-[11px] text-[#686055] mt-0.5 font-serifBody">水平質點強烈指向前鎮箱涵源</p>
                </div>
                <div class="bg-white p-3 rounded-lg border border-[#E6DEC8] cursor-pointer hover:border-[#C04A26] transition" onclick="openModal('./GMT繪圖成果/GMT_08_高雄氣爆_遠場SNJ測站空地耦合分析圖.png', '遠場 SNJ 測站 (24.8 km) 空地耦合剖析')">
                    <img src="./GMT繪圖成果/GMT_08_高雄氣爆_遠場SNJ測站空地耦合分析圖.png" class="w-full h-36 object-cover rounded">
                    <div class="mt-2 text-xs font-bold text-[#26211C] font-display">遠場空地耦合波</div>
                    <p class="text-[11px] text-[#686055] mt-0.5 font-serifBody">SNJ 測站音爆振幅為地波 6.5 倍</p>
                </div>
            </div>
        </section>

        <!-- 專題深度：2019 雲林麥寮氣爆與 2023 屏東明揚大爆炸 -->
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <!-- 麥寮專區 -->
            <div id="mailiao-study" class="bg-white rounded-xl p-7 warm-card-shadow space-y-5 border-t-4 border-t-[#2B5339]">
                <div>
                    <span class="text-xs font-mono uppercase text-[#2B5339] font-bold">Case 02 · Open-Air Blast</span>
                    <h3 class="text-xl font-bold text-[#26211C] font-display mt-1">2019 雲林麥寮氣爆：露天設備破裂與超壓音爆波</h3>
                    <p class="text-xs text-[#686055] font-mono mt-0.5">CHY 測站 (39.9 km) | 固體地波極弱，大氣超壓音爆極強</p>
                </div>
                <div class="rounded-lg overflow-hidden border border-[#E6DEC8] bg-[#F4EFE6] cursor-pointer" onclick="openModal('./GMT繪圖成果/GMT_19_2019雲林麥寮氣爆_超壓衝擊波N波解析圖.png', '麥寮氣爆超壓衝擊波 N 波微觀解析')">
                    <img src="./GMT繪圖成果/GMT_19_2019雲林麥寮氣爆_超壓衝擊波N波解析圖.png" class="w-full h-auto">
                </div>
                <p class="text-xs sm:text-sm text-[#575249] leading-relaxed font-serifBody">
                    台化芳香烴三廠 LPG 管線去丁烷塔破裂爆燃，由於發生在露天鋼構塔槽與開放空間，能量絕大部分直接向天空發散。在 39.9 km 的嘉義 CHY 測站，地下 P/S 初至波幾乎隱沒於背景地雜訊；但在第 112.7 秒時到達長達 15 秒的強烈大氣衝擊波，呈現典型空氣超音速爆轟衰減為聲波之「N 形衝擊波 (N-wave)」。
                </p>
            </div>

            <!-- 屏東明揚專區 -->
            <div id="pingtung-study" class="bg-white rounded-xl p-7 warm-card-shadow space-y-5 border-t-4 border-t-[#D67D1E]">
                <div>
                    <span class="text-xs font-mono uppercase text-[#D67D1E] font-bold">Case 03 · Delayed Double Detonation</span>
                    <h3 class="text-xl font-bold text-[#26211C] font-display mt-1">2023 屏東明揚大爆炸：過氧化物雙重爆轟連鎖殉爆</h3>
                    <p class="text-xs text-[#686055] font-mono mt-0.5">SCZ (35.6km) & SGS (48.7km) 雙測站 | 相隔 109 秒能量翻倍實證</p>
                </div>
                <div class="rounded-lg overflow-hidden border border-[#E6DEC8] bg-[#F4EFE6] cursor-pointer" onclick="openModal('./GMT繪圖成果/GMT_22_2023屏東明揚大爆炸_雙重爆轟振幅與能量放大定量比對圖.png', '明揚大爆炸雙重爆轟振幅與能量倍增比對')">
                    <img src="./GMT繪圖成果/GMT_22_2023屏東明揚大爆炸_雙重爆轟振幅與能量放大定量比對圖.png" class="w-full h-auto">
                </div>
                <p class="text-xs sm:text-sm text-[#575249] leading-relaxed font-serifBody">
                    屏東明揚科技高爾夫廠火警中，二異丙苯過氧化物（DCP）庫存引發了法醫地震學極其罕見的「雙重爆轟」。SCZ 與 SGS 兩測站精確記錄到相隔 109.2 秒的兩次連續主爆。能量積分證實第二次爆轟之振幅為第一次的 2.7 倍，累積能量暴增 7.3 倍，客觀印證第一線救災人員遭遇二次連鎖殉爆之悲痛過程。
                </p>
            </div>
        </section>

        <!-- 技術深度：Python vs GMT 差異全面剖析 (答覆使用者關鍵問題) -->
        <section id="engine-comparison" class="bg-white rounded-xl p-8 warm-card-shadow space-y-6">
            <div class="editorial-rule pb-4">
                <span class="text-xs font-mono uppercase text-[#3B5B66] font-bold tracking-wider">Cartography & Scientific Computing</span>
                <h2 class="text-2xl sm:text-3xl font-extrabold text-[#26211C] font-display mt-1">
                    技術解答：為什麼地球物理學界推崇 GMT (Generic Mapping Tools)？
                </h2>
                <p class="text-xs text-[#686055] font-mono mt-1">
                    回答「你現在是用 py 來繪圖嗎？換成 GMT 看看」的深層製圖架構差異
                </p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs sm:text-sm text-[#575249] font-serifBody leading-relaxed">
                <div class="p-4 rounded bg-[#FBF8F3] border border-[#E6DEC8] space-y-2">
                    <div class="font-bold text-[#26211C] font-display text-base">
                        <i class="fa-solid fa-compass-drafting text-[#C04A26] mr-1"></i> 1. 大地測量橢球與精確投影
                    </div>
                    <p>
                        Python (Matplotlib/Cartopy) 大地座標在繪製等震距同心環時常有投影失真；GMT 內建 WGS-84 橢球幾何演算法與全球 GSHHG 海岸線，繪製臺灣周遭等震距同心圓嚴格遵循大圓測地線（Geodesic line），無任何形變。
                    </p>
                </div>
                <div class="p-4 rounded bg-[#FBF8F3] border border-[#E6DEC8] space-y-2">
                    <div class="font-bold text-[#26211C] font-display text-base">
                        <i class="fa-solid fa-arrows-up-down text-[#3B5B66] mr-1"></i> 2. SAC 地震學剖面與倒置軸
                    </div>
                    <p>
                        國際地震學經典工具 SAC 與 GMT 深度整合，支援距離倒置 Y 軸（震央在頂端、遠場在底端）與動態標尺縮放（pswiggle / psxy），可精確還原 Yamada (2014) 第 6 頁之原汁原味出版樣式。
                    </p>
                </div>
                <div class="p-4 rounded bg-[#FBF8F3] border border-[#E6DEC8] space-y-2">
                    <div class="font-bold text-[#26211C] font-display text-base">
                        <i class="fa-solid fa-palette text-[#2B5339] mr-1"></i> 3. 時頻動態範圍與向量出版
                    </div>
                    <p>
                        GMT grdimage 搭配對數 dB 色階，能平滑呈現跨越 4 個數量級之空地耦合震波能量柱；原生 PostScript 向量輸出具備卓越抗鋸齒，完全符合 AGU、BSSA 等國際頂級期刊之圖件標準。
                    </p>
                </div>
            </div>
        </section>
        ...
        <!-- 測站結構化資料庫表格 -->
        <section id="stations-db" class="bg-white rounded-xl p-8 warm-card-shadow space-y-6">
            <div class="editorial-rule pb-4 flex items-center justify-between">
                <div>
                    <span class="text-xs font-mono uppercase text-[#A43A1B] font-bold">Observational Database</span>
                    <h2 class="text-2xl font-bold text-[#26211C] font-display mt-1">觀測測站座標與理論計算走時一覽</h2>
                </div>
                <button onclick="downloadJSON()" class="px-3.5 py-1.5 rounded bg-[#F4EFE6] text-[#26211C] hover:bg-[#C04A26] hover:text-white border border-[#E6DEC8] text-xs font-mono transition">
                    <i class="fa-solid fa-download mr-1"></i> 下載 JSON 資料庫
                </button>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse text-xs font-mono">
                    <thead>
                        <tr class="border-b border-[#26211C] bg-[#F4EFE6] text-[#26211C]">
                            <th class="py-2.5 px-3">事件歸屬</th>
                            <th class="py-2.5 px-3">測站代碼</th>
                            <th class="py-2.5 px-3">緯度 (°N)</th>
                            <th class="py-2.5 px-3">經度 (°E)</th>
                            <th class="py-2.5 px-3">震央距 (km)</th>
                            <th class="py-2.5 px-3">地殼波走時 (s)</th>
                            <th class="py-2.5 px-3">空氣波走時 (s)</th>
                            <th class="py-2.5 px-3">波形觀測特徵</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-[#E6DEC8] text-[#575249]">
                        <tr>
                            <td class="py-2 px-3 text-[#C04A26] font-bold">高雄氣爆</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">KAU</td>
                            <td class="py-2 px-3">22.566</td>
                            <td class="py-2 px-3">120.315</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">1.6</td>
                            <td class="py-2 px-3">0.46</td>
                            <td class="py-2 px-3">4.71</td>
                            <td class="py-2 px-3 font-serifBody">極近場地波與音爆重疊，PGA > 45 gal</td>
                        </tr>
                        <tr>
                            <td class="py-2 px-3 text-[#C04A26] font-bold">高雄氣爆</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">SGL</td>
                            <td class="py-2 px-3">22.618</td>
                            <td class="py-2 px-3">120.308</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">1.8</td>
                            <td class="py-2 px-3">0.51</td>
                            <td class="py-2 px-3">5.29</td>
                            <td class="py-2 px-3 font-serifBody">沿箱涵延伸方向水平運動顯著</td>
                        </tr>
                        <tr>
                            <td class="py-2 px-3 text-[#C04A26] font-bold">高雄氣爆</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">WLC</td>
                            <td class="py-2 px-3">22.568</td>
                            <td class="py-2 px-3">120.366</td>
                            <td class="py-2 px-3">7.4</td>
                            <td class="py-2 px-3">2.11</td>
                            <td class="py-2 px-3">21.76</td>
                            <td class="py-2 px-3 font-serifBody">雙震相初見分離</td>
                        </tr>
                        <tr>
                            <td class="py-2 px-3 text-[#C04A26] font-bold">高雄氣爆</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">SNJ</td>
                            <td class="py-2 px-3">22.610</td>
                            <td class="py-2 px-3">120.556</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">24.8</td>
                            <td class="py-2 px-3">7.09</td>
                            <td class="py-2 px-3">72.94</td>
                            <td class="py-2 px-3 font-serifBody">音爆空地耦合波振幅高達地波 6.5 倍</td>
                        </tr>
                        <tr>
                            <td class="py-2 px-3 text-[#2B5339] font-bold">麥寮氣爆</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">CHY</td>
                            <td class="py-2 px-3">23.500</td>
                            <td class="py-2 px-3">120.430</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">39.9</td>
                            <td class="py-2 px-3">11.40</td>
                            <td class="py-2 px-3">112.70</td>
                            <td class="py-2 px-3 font-serifBody">地波隱沒，112.7s 到達典型 N-wave 音爆</td>
                        </tr>
                        <tr>
                            <td class="py-2 px-3 text-[#D67D1E] font-bold">屏東明揚</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">SCZ</td>
                            <td class="py-2 px-3">22.715</td>
                            <td class="py-2 px-3">120.648</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">35.6</td>
                            <td class="py-2 px-3">10.17</td>
                            <td class="py-2 px-3">104.70</td>
                            <td class="py-2 px-3 font-serifBody">間隔 109s 雙重爆轟，第二次主爆振幅大 2.7 倍</td>
                        </tr>
                        <tr>
                            <td class="py-2 px-3 text-[#D67D1E] font-bold">屏東明揚</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">SGS</td>
                            <td class="py-2 px-3">22.825</td>
                            <td class="py-2 px-3">120.730</td>
                            <td class="py-2 px-3 font-bold text-[#26211C]">48.7</td>
                            <td class="py-2 px-3">13.91</td>
                            <td class="py-2 px-3">143.20</td>
                            <td class="py-2 px-3 font-serifBody">雙測站互相關交叉驗證二次連鎖爆轟源</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <!-- 頁腳 (Warm Minimalist Footer) -->
    <footer class="editorial-rule bg-[#F4EFE6] py-12 mt-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-6 text-xs text-[#686055] font-serifBody">
            <div class="space-y-1 text-center sm:text-left">
                <p class="font-bold text-[#26211C] font-display text-sm">臺灣三大重大工業氣爆地震學觀測比較研究 · 達意專題研究團隊</p>
                <p>文獻參考：Masumi Yamada et al. (2014)《從地震儀的視角看2014年高雄氣爆》· 京都大學防災研究所</p>
                <p class="font-mono text-[11px]">繪圖技術：Generic Mapping Tools 6.5.0 Modern Mode & Python ObsPy/SciPy</p>
            </div>
            <div class="flex items-center space-x-4">
                <a href="https://github.com/jimmymochi/kaohsiung-explosion-seismology" class="hover:text-[#C04A26] transition flex items-center gap-1 font-mono">
                    <i class="fa-brands fa-github text-base"></i> GitHub Pages Repo
                </a>
            </div>
        </div>
    </footer>
    ...
    <!-- 全螢幕燈箱 Modal -->
    <div id="image-modal" class="fixed inset-0 z-50 bg-[#1C1917]/90 backdrop-blur-md hidden flex flex-col items-center justify-center p-4">
        <div class="relative max-w-6xl w-full flex flex-col items-center">
            <button onclick="closeModal()" class="absolute -top-12 right-0 text-white text-3xl hover:text-[#C04A26] transition">
                &times;
            </button>
    ...
    <script>
        ...
        const themes = [
            ...
            {
                id: 8,
                title: "8. 全臺三大重大工業氣爆事件分佈總覽圖 (GMT 09)",
                desc: "GMT 高精海岸線地圖展示雲林六輕、高雄前鎮與屏東科技產業園區空間坐標與測站網絡。",
                img_gmt: "./GMT繪圖成果/GMT_09_臺灣三大重大工業氣爆事件分佈圖.png",
                img_py: "./GMT繪圖成果/GMT_05_臺灣南部測站與氣爆震央分佈地圖.png",
                analysis: [
        ...
        function setTheme(id) {
            activeThemeId = id;
            document.querySelectorAll('#gallery-tabs button').forEach((btn, idx) => {
                if (idx + 1 === id) {
                    btn.className = 'custom-tab-active px-3.5 py-2 rounded text-xs sm:text-sm font-medium border border-[#C04A26] transition bg-[#C04A26] text-white';
                } else {
                    btn.className = 'px-3.5 py-2 rounded text-xs sm:text-sm font-medium border border-[#E6DEC8] transition bg-white text-[#26211C] hover:border-[#C04A26]';
                }
            });
            renderGallery();
        }
